...
function SunriseSunsetGraphic ({sunrise, sunset}: SunriseSunsetGraphicProps) {
  ...
  return (
    <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 60' width='400'>
      <defs>
        ...
        <mask id='m1'>
          <circle cx='50' cy='50' r='33' fill='none'
            stroke='rgb(255, 255, 255)' strokeWidth='2' strokeDasharray='180'
      ...
      <svg x='6' y='51.5'>
        <TbSunrise style={{fontSize: '5px', color: textColor}} />
      </svg>
      <svg x='72' y='51.5'>
        <TbSunset style={{fontSize: '5px', color:textColor}} />
      </svg>
      <text x='17' y='56' textAnchor='middle' fill={textColor} fontSize='5'>
        {`${formatDateEpochToLocalTime(sunrise).split(':')[0]}:${formatDateEpochToLocalTime(sunrise).split(':')[1]}`}
      </text>
     
      <text x='83' y='56' textAnchor='middle' fill={textColor} fontSize='5'>
